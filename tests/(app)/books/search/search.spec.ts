...
test.describe('書籍検索', () => {
	test('簡易検索で検索結果を表示できること', async ({ loginedPage }) => {
		...
		await loginedPage.page.waitForURL('/books/search');
		...
		await loginedPage.page.waitForURL(`/books/search?query=${bookTitle}&page=0`);
	});
	...
	test('詳細検索で検索結果を表示できること', async ({ loginedPage }) => {
		...
		await input.fill(bookTitle);
		await loginedPage.page.getByTitle('検索', { exact: true }).click();
		await loginedPage.page.waitForURL(/books\/search\?booktitle=.*&author=&isbn=&page=0/);
	});
});
