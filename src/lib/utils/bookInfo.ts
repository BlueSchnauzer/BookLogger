...
export const applyChangesToBookInfos = (
	bookInfos: BookInfo[],
	detail: { message: string; updatedItem: BookInfo; deletedId: ObjectId },
	...
	if (detail.updatedItem) {
		...
		if (!isBooksRoute && oldItem?.status !== detail.updatedItem.status) {
			...
			appliedItems = [
				...bookInfos.slice(0, index),
				detail.updatedItem,
				...bookInfos.slice(index + 1)
			];
		}
	}
	if (detail.deletedId) {
