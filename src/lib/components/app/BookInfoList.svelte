	...
	{#each bookInfos as bookInfo (bookInfo._id)}
		{#if bookInfo.isVisible}
			<li style="display: inherit;" title={bookInfo.title}>
				<button class="grid h-80 max-sm:w-[128px] max-sm:h-[182px] bg-gray-100 rounded shadow-md" on:click={() => handleClick(bookInfo)}>
...
{#if isDisplayDetail}
	<BookInfoDetail item={currentBookInfo} bind:isDisplay={isDisplayDetail}/>
{/if}
