import type { bookInfoEvent, updateBookInfoParameter, deletionBookInfoParameter } from "$lib/client/Utils/customEvent/dispatcher";
...
export const handleUpdateSuccess = (bookInfoViews: BookInfoView[], paramter: updateBookInfoParameter, targetToast: string, isBooksRoute = false) => {
  ...
  pushSuccessToast(paramter.message, targetToast);

  return appliedViews;
}

export const handleDeletionSuccess = (bookInfoViews: BookInfoView[], parameter: deletionBookInfoParameter, targetToast: string) => {
  //一旦、viewの中身を取り出して新しく操作する、上手くいくか確認
  let copiedViews = [...bookInfoViews];
  let appliedViews: BookInfoView[] = [];
  appliedViews = copiedViews.filter(item => item.id?.equals(parameter.deletedId));

  pushSuccessToast(parameter.message, targetToast);

  return appliedViews;
}

export const handleFailure = (message: string, targetToast: string) => {
  pushErrorToast(message, targetToast);
}
