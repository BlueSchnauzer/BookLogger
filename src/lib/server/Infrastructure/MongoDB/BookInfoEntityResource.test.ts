...
describe('get', () => {
  it('SvelteAPIへのリクエストが成功した際に、レスポンスをEntityに変換して戻り値で返すこと', async () => {
    ...
    expect(response[0].getTitle()).toEqual(dbModels[0].title);
  });
...
describe('getByStatus', () => {
  ...
  it('statusにwishを指定してリクエストした際に、対応したレスポンスを戻り値で返すこと', async () => {
    ...
    expect(response.length).toEqual(1);
    expect(response[0].getStatus().value).toEqual('wish');
  });
  ...
  it('statusにreadingを指定してリクエストした際に、対応したレスポンスを戻り値で返すこと', async () => {
    ...
    expect(response.length).toEqual(1);
    expect(response[0].getStatus().value).toEqual('reading');
  });
  ...
  it('statusにcompleteを指定してリクエストした際に、対応したレスポンスを戻り値で返すこと', async () => {
    ...
    expect(response.length).toEqual(1);
    expect(response[0].getStatus().value).toEqual('complete');
  });
});
...
describe('getRecent', () => {
  ...
  it('SvelteAPIへのリクエストが成功した際に、レスポンスをEntityに変換して戻り値で返すこと', async () => {
    ...
    expect(response!.getPageHistories()?.length).toEqual(2);
  });
});
