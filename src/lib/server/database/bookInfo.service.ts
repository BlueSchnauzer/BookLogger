...
export async function getRecentBookInfo(
	...
	try {
		...
		bookInfos = (await collections.bookInfos
			?.find({
				userId,
...
export async function getBookInfoWithOnlyPageHistory(collections: collections, userId: string) {
	...
	try {
		...
		histories = (await collections.bookInfos
			?.find({ userId })
			.project(projection)
			.toArray()) as BookInfo[];
	} catch (error) {
...
export async function getBookInfoByStatus(
	collections: collections,
	userId: string,
	status: status
): Promise<BookInfo[]> {
...
/**書誌データを保存する */
export async function insertBookInfo(
	collections: collections,
	bookInfo: BookInfo
): Promise<Response> {
...
/**同様の書誌データが既に保存されているか */
export async function isDuplicateBookInfo(
	collections: collections,
	userId: string,
	gapiId: string
): Promise<boolean> {
	...
	try {
		const bookInfos = (await collections.bookInfos
			?.find({ userId, gapiId })
			.toArray()) as BookInfo[];
...
/**書誌データを削除する */
export async function deleteBookInfo(
	collections: collections,
